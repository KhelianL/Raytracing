...
static GLint window;
static unsigned int SCREENWIDTH = PARAM_WINDOW_WIDTH;
static unsigned int SCREENHEIGHT = PARAM_WINDOW_HEIGHT;
...
float percent;
Vec3 tabDir[PARAM_WINDOW_WIDTH][PARAM_WINDOW_HEIGHT][PARAM_NUMBER_SAMPLES];
Vec3 pos;
std::vector<Vec3> image(PARAM_WINDOW_WIDTH *PARAM_WINDOW_HEIGHT, Vec3(0, 0, 0));

void func(int y)
{
    for (int x = 0; x < (int)PARAM_WINDOW_WIDTH; x++)
    {
        for (unsigned int s = 0; s < PARAM_NUMBER_SAMPLES; ++s)
        {
            Vec3 color = scenes[selected_scene].rayTrace(Ray(pos, tabDir[x][y][s]));

            image[x + y * PARAM_WINDOW_WIDTH] += color;
        }
        image[x + y * PARAM_WINDOW_WIDTH] /= PARAM_NUMBER_SAMPLES;
    }

    // Complétion
    percent += (1.0 / PARAM_WINDOW_HEIGHT) * 100;
    cout << percent << "%" << endl;
}

void ray_trace_from_camera()
{
    int w = PARAM_WINDOW_WIDTH;
    int h = PARAM_WINDOW_HEIGHT;
    ...
    thread everyThread[PARAM_WINDOW_HEIGHT];
    percent = 0;
    ...
    for (int y = 0; y < h; y++)
    {
        ...
        for (int x = 0; x < w; x++)
        {
            for (unsigned int s = 0; s < PARAM_NUMBER_SAMPLES; ++s)
            {
                ...
                tabDir[x][y][s] = screen_space_to_worldSpace(u, v) - pos;
                tabDir[x][y][s].normalize();
            }
        }

        // Lancement Thread sur chaque ligne width
        everyThread[y] = thread(func, y);
    }

    // Join Threads
    for (int i = 0; i < (int)PARAM_WINDOW_HEIGHT; i++)
    {
        everyThread[i].join();
    }
    ...
    // Reset img
    image.clear();
    image.resize(PARAM_WINDOW_WIDTH * PARAM_WINDOW_HEIGHT);
...
int main(int argc, char **argv)
{
    ...
    scenes[4].setup_giant_cornell_box();
    scenes[5].setup_double_light();
    scenes[6].setup_mesh();
