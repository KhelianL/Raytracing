...
public:
    ...
    RayTriangleIntersection intersect(Ray const &ray) const
    {
        ...
        for (int i = 0, maxSize = triangles.size(); i < maxSize; i++)
        {
            // Création Triangle
            Vec3 v1 = vertices[triangles[i][0]].position;
            Vec3 v2 = vertices[triangles[i][1]].position;
            Vec3 v3 = vertices[triangles[i][2]].position;
            Triangle tmpTriangle = Triangle(v1, v2, v3);

            // Intersection triangle-ray
            RayTriangleIntersection tmpData = tmpTriangle.getIntersection(ray);

            // Plus proche?
            if (tmpData.intersectionExists && tmpData.t < closestIntersection.t && closestIntersection.t >= 0)
            {
                closestIntersection.intersectionExists = true;
                closestIntersection.intersection = tmpData.intersection;
                closestIntersection.w0 = tmpData.w0;
                closestIntersection.w1 = tmpData.w1;
                closestIntersection.w2 = tmpData.w2;
                closestIntersection.t = tmpData.t;
                closestIntersection.normal = tmpData.normal;
            }
        }
